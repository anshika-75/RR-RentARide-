  ...
  return (
    <div className="px-4 pt-10 md:px-10 flex-1">
      <Title
        ...
        subTitle="Monitor overall platform performance including total cars.bookings etc"
      />
      <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 my-8 max-w-3xl">
        {dashboardCards.map((value, index) => (
          ...
          >
            <div>
              <h1 className="text-xs text-gray-500">{value.title}</h1>
              <p className="text-lg font-semibold">{value.value}</p>
            </div>
            <div className="flex items-center justify-center w-10 h-10 rounded-full bg-primary/10">
              <img src={value.icon} className="w-4 h-4 " />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-start gap-6 mb-8 w-full">
        <div className="p-4 md:p-6 border border-borderColor rounded-md max-w-lg w-full">
          ...
          <p className="text-gray-500">Latest Cutomer Bookings</p>
